...
Run `react-native link react-native-video-exp` to link the react-native-video-exp library.
...
`react-native link react-native-video-exp` doesn’t work properly with the tvOS target so we need to add the library manually.
...
```gradle
include ':react-native-video-exp'
project(':react-native-video-exp').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-video-exp/android-exoplayer')
```
...
```gradle
include ':react-native-video-exp'
project(':react-native-video-exp').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-video-exp/android')
```
...
- UWP: Select `node_modules\react-native-video-exp\windows\ReactNativeVideo\ReactNativeVideo.csproj`
- WPF: Select `node_modules\react-native-video-exp\windows\ReactNativeVideo.Net46\ReactNativeVideo.Net46.csproj`
...
import RCTVideoManager from "react-native-video-exp/dom/RCTVideoManager"; // Add this
